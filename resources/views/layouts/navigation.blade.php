<nav x-data="{ isMenuOpen: false, isProfileOpen: false, isSwitcherOpen: false, isSettingsOpen: false }" class="bg-gray-800">
        ...
        <div class="relative flex items-center justify-between h-16">
            ...
            <x-navigation.menu>
                ...
                <x-slot name="links">
                    ...
                    <x-navigation.link href="{{ route('users.index') }}" active="{{ request()->routeIs('users*') }}">
                        {{ __('language.users') }}
                    </x-navigation.link>
                    <x-navigation.link href="{{ route('employees.index') }}" active="{{ request()->routeIs('employees*') }}">
                        {{ __('language.employees') }}
                    </x-navigation.link>
                    <x-navigation.link href="{{ route('hotels.index') }}" active="{{ request()->routeIs('hotels*') }}">
                        {{ __('language.hotels') }}
                    </x-navigation.link>
                    <x-navigation.link href="{{ route('sheets.index') }}" active="{{ request()->routeIs('sheets*') }}">
                        {{ __('language.sheets') }}
                    </x-navigation.link>
                </x-slot>
    ...
    <x-navigation.responsive>
        ...
        <x-slot name="links">
            ...
            <x-navigation.link class="block text-base" href="{{ route('users.index') }}" active="{{ request()->routeIs('users*') }}">
                {{ __('language.users') }}
            </x-navigation.link>
            <x-navigation.link class="block text-base" href="{{ route('employees.index') }}" active="{{ request()->routeIs('employees*') }}">
                {{ __('language.employees') }}
            </x-navigation.link>
            <x-navigation.link class="block text-base" href="{{ route('hotels.index') }}" active="{{ request()->routeIs('hotels*') }}">
                {{ __('language.hotels') }}
            </x-navigation.link>
            <x-navigation.link class="block text-base" href="{{ route('sheets.index') }}" active="{{ request()->routeIs('sheets*') }}">
                {{ __('language.sheets') }}
            </x-navigation.link>
        </x-slot>
